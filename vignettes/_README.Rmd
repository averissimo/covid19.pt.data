...
print(age.data.all.new %>%
  ...
  labs(title = 'New {format(label.death$women + label.death$men, big.mark = ",", trim = TRUE)} deaths from {format(age.ix.f, "%B %d")}' %>% glue,
       subtitle = '',
       caption = '',
       y = 'Age group',
       x = 'Deaths') +
...
```{r, result='asis', echo=FALSE,fig.width=4.4}
...
print(age.data.all %>%
  ...
  labs(title = 'Total number of Confirmed cases ({format(label.confirmed$women + label.confirmed$men, big.mark = ",", trim = TRUE)}) by age group' %>% glue,
       y = 'Age group',
       subtitle = '',
       caption = '',
       x = 'Confirmed Cases') +
...
print(age.data.all %>%
  ...
  scale_x_continuous('', labels = function(ix) { return(abs(ix)) }) +
  scale_fill_viridis_d('', end = .8, labels = function(ix) { return(paste0(ix, ': ', format(label.death[ix], big.mark = ',', trim = TRUE), ''))}) + 
  labs(title = 'Total number of Deaths ({format(label.death$women + label.death$men, big.mark = ",", trim = TRUE)}) by age group' %>% glue,
