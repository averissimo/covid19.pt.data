...
}, error = function(err) { cat('Error:: Some data for covid-19 demographic might not be available from DGS\n')})
```


```{r, echo=FALSE,fig.width=4.4}
if (nrow(age.data.all.new) > 0) {
  ...
  if (confirmed.status) {
    confirmed.labs$title <- 'ERROR: no DGS data for demographics of new confirmed cases'
  ...
  if (death.status) {
    death.labs$title <- 'ERROR:: no DGS data for demographics of deaths'
    death.labs$caption <- ''
  }
  #
  #
  ...
  cat('There\'s a problem with the data from dgs, we can\'t display demographics data.\n')
}
```
...
if (nrow(age.data.all) > 0) {
  ...
  confirmed.status <- sum(abs(age.data.all.new$confirmed)) != covid19.pt %>% filter(dateRep == strftime(anydate(age.ix.f)+1, '%d/%m/%Y')) %>% pull(cases) %>% sum
  if (confirmed.status) {
    confirmed.labs$title <- 'ERROR:: no DGS data for demographics of total confirmed cases'
    confirmed.labs$caption <- ''
  }
  #
  ...
  death.status <- sum(abs(age.data.all.new$death)) != covid19.pt %>% filter(dateRep == strftime(anydate(age.ix.f)+1, '%d/%m/%Y')) %>% pull(deaths) %>% sum
  if (death.status) {
    death.labs$title <- 'ERROR:: no DGS data for demographics of total deaths'
    death.labs$caption <- ''
  }
  #
  #
